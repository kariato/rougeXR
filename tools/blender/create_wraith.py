"""Original spectral wraith. Blender 4.5 LTS; no external assets."""
import bpy,math
from pathlib import Path
from mathutils import Vector
R=Path(__file__).resolve().parents[2];N='wraith';S=R/f'art/blender/creatures/{N}.blend';E=R/f'public/assets/creatures/{N}.glb';P=R/f'art/previews/{N}.png'
...
cloak=mat('wraith_cloak',(.08,.09,.15));edge=mat('spectral_edges',(.25,.38,.49));void=mat('wraith_void',(.008,.012,.025));glow=mat('soul_glow',(.24,.76,.88));bone=mat('wraith_bone',(.52,.60,.61));parts=[]
...
# Hovering faceless apparition, facing Blender -Y.
