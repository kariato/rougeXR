"""Original forest nymph. Blender 4.5 LTS; no external assets."""
import bpy,math
from pathlib import Path
from mathutils import Vector
R=Path(__file__).resolve().parents[2];N='nymph';S=R/f'art/blender/creatures/{N}.blend';E=R/f'public/assets/creatures/{N}.glb';P=R/f'art/previews/{N}.png'
...
skin=mat('nymph_skin',(.63,.43,.29));leaf=mat('leaf_dress',(.08,.34,.16));light=mat('young_leaves',(.28,.57,.22));hair=mat('willow_hair',(.16,.10,.035));gold=mat('stolen_charm',(.88,.58,.08),.65);eye=mat('green_eyes',(.18,.65,.39));parts=[]
...
# Slender woodland thief, facing Blender -Y.
...
part('charm',( .27,-.10,.91),(.06,.025,.075),gold,'arm.L');part('charm_gem',(.27,-.128,.91),(.025,.012,.03),eye,'arm.L')
...
def attack(t):q=math.sin(t*math.pi)**2;rig.pose.bones['arm.L'].rotation_euler.x=-1.35*q;rig.pose.bones['spine'].rotation_euler.z=.25*q;rig.pose.bones['root'].location.y=-.08*q
