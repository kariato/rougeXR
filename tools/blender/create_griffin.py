"""Original eagle-lion griffin. Blender 4.5 LTS; no external assets."""
import bpy,math
from pathlib import Path
from mathutils import Vector
ROOT=Path(__file__).resolve().parents[2];N='griffin';S=ROOT/f'art/blender/creatures/{N}.blend';E=ROOT/f'public/assets/creatures/{N}.glb';P=ROOT/f'art/previews/{N}.png'
...
brown=mat('griffin_lion_fur',(.48,.29,.09));cream=mat('griffin_eagle_feathers',(.66,.52,.25));stripe=mat('griffin_wing_tips',(.18,.10,.045));dark=mat('griffin_beak_talons',(.055,.035,.015));eye=mat('griffin_eyes',(.90,.55,.06));horn=mat('griffin_beak',(.72,.46,.08));parts=[]
...
# Griffin with lion hindquarters and eagle forequarters; faces Blender -Y.
part('body',(0,.12,.72),(.29,.55,.28),brown,'body');part('chest',(0,-.31,.79),(.27,.27,.34),cream,'body');part('neck',(0,-.42,1.04),(.19,.22,.38),cream,'neck');part('head',(0,-.61,1.25),(.18,.30,.18),brown,'head');part('eagle_face',(0,-.80,1.23),(.16,.20,.16),cream,'head');cone('hooked_beak',(0,-.87,1.22),(0,-1.12,1.15),.105,horn,'head')
for i in range(7): cone(f'neck_feather.{i}',((i-3)*.045,-.43,1.18),((i-3)*.065,-.28,1.00),.055,cream,'neck')
...
part('wing.L',(.38,.14,1.02),(.16,.48,.18),cream,'body');part('wing.R',(-.38,.14,1.02),(.16,.48,.18),cream,'body');cone('wingtip.L',(.42,.30,1.02),(.88,.68,.90),.16,stripe,'body');cone('wingtip.R',(-.42,.30,1.02),(-.88,.68,.90),.16,stripe,'body')
