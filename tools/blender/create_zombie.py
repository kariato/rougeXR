"""Original shambling zombie. Blender 4.5 LTS; no external assets."""
import bpy
import math
...
SOURCE=ROOT/'art/blender/creatures/zombie.blend'
EXPORT=ROOT/'public/assets/creatures/zombie.glb'
PREVIEW=ROOT/'art/previews/zombie.png'
...
skin=material('zombie_dead_skin',(.29,.36,.25)); rot=material('zombie_decay',(.18,.14,.075))
bruise=material('zombie_bruises',(.20,.09,.16)); shirt=material('zombie_torn_shirt',(.20,.25,.22))
trouser=material('zombie_trousers',(.15,.115,.08)); blood=material('zombie_dried_blood',(.27,.025,.018))
bone_mat=material('zombie_exposed_bone',(.74,.68,.49)); eye=material('zombie_sick_eye',(.75,.78,.34))
dark=material('zombie_empty_socket',(.018,.012,.012)); parts=[]
...
# Crooked 1.7 m humanoid facing Blender -Y. Deliberate left/right asymmetry sells decay.
...
part('missing_cheek',(-.12,-.17,1.34),(.09,.065,.11),rot,'head')
...
part('socket.R',(-.055,-.211,1.44),(.055,.028,.052),dark,'head')
part('brow_wound',(-.055,-.238,1.49),(.085,.012,.022),blood,'head','cube')
...
cone('exposed_shin',(-.13,-.07,.31),(-.13,-.06,.12),.035,bone_mat,'leg.R')

bpy.ops.object.select_all(action='DESELECT')
for obj in parts: obj.select_set(True)
bpy.context.view_layer.objects.active=parts[0]; bpy.ops.object.join()
mesh=bpy.context.object; mesh.name='zombie_mesh'; scene.cursor.location=(0,0,0); bpy.ops.object.origin_set(type='ORIGIN_CURSOR')
armature=bpy.data.armatures.new('zombie_skeleton'); rig=bpy.data.objects.new('zombie_rig',armature)
