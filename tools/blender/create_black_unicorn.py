"""Original black unicorn. Blender 4.5 LTS; no external assets."""
import bpy,math
from pathlib import Path
from mathutils import Vector
ROOT=Path(__file__).resolve().parents[2];N='black-unicorn';S=ROOT/f'art/blender/creatures/{N}.blend';E=ROOT/f'public/assets/creatures/{N}.glb';P=ROOT/f'art/previews/{N}.png'
...
brown=mat('unicorn_black_hide',(.025,.022,.035));cream=mat('unicorn_violet_sheen',(.12,.07,.18));stripe=mat('unicorn_runes',(.36,.14,.48));dark=mat('mane_hooves',(.015,.012,.020));eye=mat('eyes',(.72,.12,.70));horn=mat('spiral_horn',(.66,.58,.78));parts=[]
...
# Ominous black unicorn with violet markings; faces Blender -Y.
...
cone('horn',(0,-.61,1.38),(0,-.96,1.66),.065,horn,'head')
for i in range(4):part(f'rune.{i}',((i-1.5)*.10,-.575,.88+(i%2)*.12),(.025,.018,.05),stripe,'body')
