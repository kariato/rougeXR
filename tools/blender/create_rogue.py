"""Original stylized player rogue. Blender 4.5 LTS; no external assets."""
import bpy, math
from pathlib import Path
from mathutils import Vector
ROOT=Path(__file__).resolve().parents[2]; NAME='rogue'
...
green=mat('tunic_blue',(.055,.15,.24)); gold=mat('buckle_brass',(.88,.52,.06),.75); steel=mat('dagger_steel',(.48,.53,.57),.8); red=mat('hood_lining',(.08,.09,.11)); skin=mat('warm_skin',(.62,.38,.22)); brown=mat('leather',(.18,.07,.025)); cream=mat('shirt',(.72,.68,.48)); black=mat('hood_boots',(.025,.03,.04)); eye=mat('eyes',(.15,.32,.48)); parts=[]
...
part('head',(0,-.02,.82),(.18,.15,.17),skin,'head'); part('nose',(0,-.19,.82),(.05,.065,.05),skin,'head'); part('face_shadow',(0,-.15,.78),(.14,.04,.10),black,'head')
part('hood',(0,.01,.94),(.22,.18,.24),black,'head'); cone('hood_tail',(0,.10,1.02),(0,.18,.72),.12,black,'head',8)
...
part('dagger_grip',(-.25,-.02,.38),(.035,.035,.13),brown,'arm.R','cube'); cone('dagger_blade',(-.25,-.02,.29),(-.25,-.02,.05),.055,steel,'arm.R',8); part('dagger_guard',(-.25,-.02,.29),(.10,.025,.025),steel,'arm.R','cube')
