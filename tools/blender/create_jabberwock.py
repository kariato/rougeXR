"""Original uncanny jabberwock dragonkin. Blender 4.5 LTS; no external assets."""
import bpy,math
from pathlib import Path
from mathutils import Vector
ROOT=Path(__file__).resolve().parents[2];N='jabberwock';S=ROOT/f'art/blender/creatures/{N}.blend';E=ROOT/f'public/assets/creatures/{N}.glb';P=ROOT/f'art/previews/{N}.png'
...
brown=mat('jabberwock_scales',(.18,.30,.16));cream=mat('jabberwock_belly',(.42,.55,.24));stripe=mat('jabberwock_wings',(.30,.12,.34));dark=mat('jabberwock_horns_claws',(.055,.035,.08));eye=mat('jabberwock_eyes',(.95,.12,.07));horn=mat('jabberwock_teeth',(.82,.76,.55));parts=[]
...
# Uncanny long-necked dragonkin with jaws, whiskers, horns and mismatched wings; faces Blender -Y.
part('body',(0,.12,.72),(.29,.55,.28),brown,'body');part('chest',(0,-.31,.79),(.27,.27,.34),cream,'body');part('neck',(0,-.42,1.04),(.19,.22,.38),cream,'neck');part('head',(0,-.61,1.25),(.18,.30,.18),brown,'head');part('long_snout',(0,-.84,1.22),(.17,.28,.14),brown,'head');part('lower_jaw',(0,-.91,1.12),(.16,.24,.08),cream,'head');cone('fang.L',(.08,-1.02,1.18),(.08,-1.04,1.08),.025,horn,'head');cone('fang.R',(-.08,-1.02,1.18),(-.08,-1.04,1.08),.025,horn,'head');cone('whisker.L',(.13,-.96,1.22),(.48,-1.18,1.30),.012,horn,'head');cone('whisker.R',(-.13,-.96,1.22),(-.48,-1.18,1.30),.012,horn,'head')
for i in range(7):cone(f'neck_spine.{i}',(0,-.24+i*.07,1.20-i*.035),(0,-.16+i*.07,1.38-i*.03),.045,stripe,'neck')
for side,sgn in [('L',1),('R',-1)]:part('eye.'+side,(sgn*.14,-.73,1.31),(.038,.025,.038),eye,'head');cone('horn.'+side,(sgn*.09,-.49,1.38),(sgn*.16,-.39,1.66),.055,dark,'head')
...
part('wing.L',(.38,.14,1.02),(.18,.55,.12),stripe,'body');part('wing.R',(-.38,.14,1.02),(.14,.42,.12),stripe,'body');cone('wingtip.L',(.42,.30,1.02),(.88,.68,.90),.16,stripe,'body');cone('wingtip.R',(-.42,.30,1.02),(-.88,.68,.90),.16,stripe,'body')
