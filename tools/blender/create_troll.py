"""Original mossy cave troll. Blender 4.5 LTS; no external assets."""
import bpy,math
from pathlib import Path
from mathutils import Vector
R=Path(__file__).resolve().parents[2];N='troll';S=R/f'art/blender/creatures/{N}.blend';E=R/f'public/assets/creatures/{N}.glb';P=R/f'art/previews/{N}.png'
...
fur=mat('troll_moss_hide',(.22,.34,.12));shadow=mat('troll_dark_hide',(.10,.19,.07));skin=mat('troll_stone_skin',(.34,.38,.25));dark=mat('troll_claws',(.055,.045,.025));eye=mat('troll_amber_eyes',(.90,.43,.06));tooth=mat('troll_tusks',(.78,.68,.43));parts=[]
...
# Massive mossy cave brute, facing Blender -Y.
...
cone('tusk.L',(.12,-.45,1.39),(.16,-.49,1.24),.035,tooth,'jaw');cone('tusk.R',(-.12,-.45,1.39),(-.16,-.49,1.24),.035,tooth,'jaw')
for i in range(5):part(f'wart.{i}',((i-2)*.07,-.43,1.58+(i%2)*.06),(.018,.012,.018),shadow,'head')
