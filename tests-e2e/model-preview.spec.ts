...
for (const model of ['hobgoblin', 'kestrel', 'rattlesnake', 'bat', 'snake', 'ice-monster', 'orc', 'zombie', 'leprechaun', 'centaur', 'quagga', 'aquator', 'nymph', 'yeti', 'venus-flytrap', 'troll', 'wraith', 'phantom', 'xeroc', 'black-unicorn']) test(`loads the Blender ${model} and plays each exported clip`, async ({ page }) => {
  const errors: string[] = [];
