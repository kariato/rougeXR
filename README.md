...
Current implementation: the Phase 11 rules expansion is playable in the regular browser. The default New Game path creates a deterministic generated dungeon from a user-supplied seed; explicit Kestrel and two-room fixtures remain available for focused debugging. Generation now includes the complete source monster and object tables, treasure rooms, wandering monsters, the Amulet, ascent, and victory. The build remains labeled as a supported slice until the Phase 11 command and behavior audit is complete.
