...
Current implementation: the Phase 11 rules expansion is playable in the regular browser. The default New Game path creates a deterministic generated dungeon from a user-supplied seed; explicit Kestrel and two-room fixtures remain available for focused debugging. Generation now uses the complete source monster and ordinary object tables, including species abilities and monster packs. Treasure rooms, wandering spawns, and Amulet progression are still tracked work, so the build remains labeled as a supported slice.
