...
Current implementation: the Phase 11 full-rules candidate is playable in the regular browser, with Phase 12 desktop 3D underway. First-person 3D is the default presentation and the original map remains available from the view selector without restarting the session. The default New Game path creates a deterministic generated dungeon from a user-supplied seed; explicit Kestrel and two-room fixtures remain available for focused debugging. Generation includes the complete source monster and object tables, treasure rooms, wandering monsters, the Amulet, ascent, and victory. Browser replacements for terminal commands are recorded in the command audit.
